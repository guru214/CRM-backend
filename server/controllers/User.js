...
const Profile = async (req, res) => {
  try {
    ...
    const [rows] = await mySqlPool.query(
      "SELECT FullName, Email, Phone, Account_Type, Address, documentType, documentNumber FROM users WHERE id = ?",
      [id]
    );
  ...
  const forgotPassword = async (req, res) => {
    try {
        ...
        const [user] = await mySqlPool.query("select email from users where Email = ?",[Email])
        if (!user[0]) {
            return res.status(404).json({ message: "User not found" });
        }
...
const resetPassword = async (req, res, next) => {
    try {
        ...
        // Verify token and also it gives me here the id so i can get that user from the models directly using that id
        const decoded = jwt.verify(token, process.env.JWT_SECRET_KEY);
        console.log("Decoded Token:", decoded);

        // Find user by decoded token
        const user = await Employer.findById(decoded.id) || await Freelancer.findById(decoded.id);
        if (!user) {
            return res.status(404).json({ message: "User not found" });
        }
...
export { Register, Login, Profile, UpdateProfile, ChangePassword, KYCUpdate, Logout };
